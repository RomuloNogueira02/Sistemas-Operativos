...
def procuraPalavras(palavra, ficheiro):
    ...
    for tuplo in ficheiroLido:
        ocorrencias.append(
            ...
            flags= re.IGNORECASE), tuplo[1]))# Isto tudo para retirar os acentos
...
def verificacao (listaPalavras, tuplo):
    ...
    """
    var = True
    dadosAVerificar = list(map(lambda palavra: unicodedata.normalize("NFD", palavra).encode("ascii","ignore").decode("utf-8").lower() ,tuplo[0]))
    palavrasAVerificar = list(map(lambda palavra:unicodedata.normalize("NFD", palavra).encode("ascii","ignore").decode("utf-8").lower() ,listaPalavras))
...
###################################################################################################################
def criaThreads(nFilhas, funcao, listaPalavras, listaFicheiros):
    """
    Função que cria threads filhas

    Args:
        nFilhos (int): número de threads a criar
        funcao (?): Função alvo do threads
        listaPalavras (List): Lista de palavras
        listaFicheiro (List): Lista de ficheiros
    """
    listaThreads = []

    for i in range(nFilhas):
        listaThreads.append(Thread(target = funcao, args = (listaPalavras, listaFicheiros[i], )))
...
def recebeComandos():
    ...
    if len(palavras) > 3:
        ...
        pgrepwc (palavras, ficheiros, comandos, nThreads)

###################################################################################################################
# OPÇÃO -a                                                                                                        #
###################################################################################################################
def opcaoA (listaPalavras, listaFicheiro):
    """
    ...
    """
    ocorrencias = []
    ficheirosLidos = [leFicheiro(ficheiro) for ficheiro in listaFicheiro]
...
def opcaoLsemA (listaPalavras,listaFicheiros):
    ...
    contadorDeLinhas += sum(map(lambda tuplo: tuplo[1],numLinhasPorPalavra))
...
def opcaoLcomA(listaPalavras,listaFicheiros):
    ...
    contadorDeLinhas += len(listaFinal)
...
def opcaoCcomA(listaPalavras,listaFicheiros):
    ...
    contadorDeOcorrencias += sum(map(lambda tuplo : tuplo[1], ocorrenciasPorPalavra ))
...
# FUNÇÃO PRINCIPAL                                                                                                #
###################################################################################################################
def pgrepwc (listaPalavras, listaFicheiros, listaOpcoes, nThreads):
    """
    Função que ...
    Args:
        listaPalavras (List): Lista de palavras
        ...
        nThreads (int): Número de threads a serem criadas
    """
    if ("-p" in listaOpcoes):
        ...
        if nFilhas > nFicheiros:
            nFilhas = nFicheiros
        
        if nFilhas < nFicheiros:
            novaListaFicheiros = parteLista(listaFicheiros, nFilhas)

        if "-l" in listaOpcoes:

            if "-a" not in listaOpcoes: 
                criaThreads(nFilhas, opcaoLsemA, listaPalavras, novaListaFicheiros)
            
            else:
                criaThreads(nFilhas, opcaoLcomA, listaPalavras, novaListaFicheiros)

            print("A pesquisa resultou em: " + str(contadorDeLinhas) + " linhas")

        elif "-c" in listaOpcoes:

            if "-a" not in listaOpcoes:
                criaThreads(nFilhas, opcaoCsemA, listaPalavras, novaListaFicheiros)
            else: 
                criaThreads(nFilhas, opcaoCcomA, listaPalavras, novaListaFicheiros)

            print("A pesquisa resultou em: " + str(contadorDeOcorrencias) + " ocorrências das palavras")
